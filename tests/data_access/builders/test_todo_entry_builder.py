...
class TestTodoEntryBuilder:
    ...
    def test_set_date_time_parses_string_with_configured_format(self, mocker):
        mocker.patch("src.data_access.builders.todo_builder.get_config", return_value={"DATETIME_FORMAT": "something"})
        expected_datetime_parsed = datetime.today()
        mocked_datetime = mocker.patch('src.data_access.builders.todo_builder.dt')
    ...
    def test_set_date_throws_bad_server_configuration_server_on_missing_configuration(self, mocker):
        mocker.patch('src.data_access.builders.todo_builder.dt')
        mocker.patch("src.data_access.builders.todo_builder.get_config", return_value={})
